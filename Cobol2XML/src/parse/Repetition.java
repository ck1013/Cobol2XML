...
public class Repetition extends Parser {
     ...
     * @return a repetiton that will match the given parser repeatedly in successive matches
     */
     ...
     * <p>
     * For example, matching the regular expression <code>a*</code> against <code>{^aaab}</code> results in <code>{^aaab, a^aab, aa^ab, aaa^b}</code>.
     *
     * @return ArrayList of assemblies that result from matching against a beginning set of assemblies
    ...
    public ArrayList<Assembly> match(ArrayList<Assembly> in) {
        if (preAssembler != null) {
            ...
            while (e.hasMoreElements()) {
                preAssembler.workOn(e.nextElement());
            }
        }
     ...
     * @return ArrayList of random assemblies
     */
    ...
    protected ArrayList<Assembly> randomExpansion(int maxDepth, int depth) {
        ...
        for (int j = 0; j < n; j++) {
            ArrayList<Assembly> w = (ArrayList<Assembly>) subparser.randomExpansion(maxDepth, depth++);
            Enumeration<Assembly> e = Collections.enumeration(w);
            while (e.hasMoreElements()) {
                v.add(e.nextElement());
            }
        }
